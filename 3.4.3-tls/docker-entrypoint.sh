...
if [ "$1" = '/opt/couchdb/bin/couchdb' ]; then
	...
	if [[ -f $COUCHDB_TLS_KEY_PATH ]] && [[ -f $COUCHDB_TLS_CERT_PATH ]] && [[ -f $COUCHDB_TLS_CACERT_PATH ]]; then
		sed -i '/\[ssl\]/,/^\[/ s/^[[:space:]]*;enable[[:space:]]*=[[:space:]]*true/enable = true/' /opt/couchdb/etc/local.ini
		sed -i "s#;cert_file = .*#cert_file = $COUCHDB_TLS_CERT_PATH#g" /opt/couchdb/etc/local.ini
		sed -i "s#;key_file = .*#key_file = $COUCHDB_TLS_KEY_PATH#g" /opt/couchdb/etc/local.ini
		sed -i "s#;cacert_file = .*#cacert_file = $COUCHDB_TLS_CACERT_PATH#g" /opt/couchdb/etc/local.ini
	fi	
